...
pub fn encrypt(kind: PacketKind, key: &[u8; 32], nonce: &[u8; 12], plaintext: &[u8])
    -> Result<Vec<u8>, String>
{
    let aead = ChaCha20Poly1305::new(Key::from_slice(key));
    let nonce = Nonce::from_slice(nonce);
    let payload = Payload { msg: plaintext, aad: &aad(kind) };
    aead.encrypt(nonce, payload).map_err(|_| "encrypt_failed".to_string())
}

/// Decrypt/verify with AAD bound to the `PacketKind`.
pub fn decrypt(kind: PacketKind, key: &[u8; 32], nonce: &[u8; 12], ciphertext: &[u8])
    -> Result<Vec<u8>, String>
{
    let aead = ChaCha20Poly1305::new(Key::from_slice(key));
    let nonce = Nonce::from_slice(nonce);
    let payload = Payload { msg: ciphertext, aad: &aad(kind) };
    aead.decrypt(nonce, payload).map_err(|_| "auth_failed".to_string())
}
