...
static HSIP_KEY: Lazy<Vec<u8>> = Lazy::new(|| {
    let k = std::env::var("HSIP_LOCAL_JWT_KEY_HEX")
        .unwrap_or_else(|_| "00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff".to_string());
    hex::decode(k).expect("bad HSIP_LOCAL_JWT_KEY_HEX")
});

#[derive(Serialize)]
struct Status { ok: bool, version: &'static str }

#[derive(Deserialize)]
struct TokenReq { aud: Option<String> }
...
pub async fn run_identity_broker() -> anyhow::Result<()> {
    let app = Router::new()
        .route("/status", get(|| async { Json(Status { ok: true, version: "0.2.0-mvp" }) }))
        .route("/token",  post(token))
        .route("/demo",   get(demo))
        .layer(DefaultBodyLimit::disable());

    let addr_str = std::env::var("HSIP_IDENTITY_ADDR").unwrap_or_else(|_| "127.0.0.1:9100".to_string());
...
async fn token(Json(req): Json<TokenReq>) -> impl IntoResponse {
    ...
    let signed = Token::new(Header::default(), claims).sign_with_key(&key).unwrap();
